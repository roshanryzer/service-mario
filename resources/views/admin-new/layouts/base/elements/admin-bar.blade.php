<nav id="admin_bar">
    <div class="admin-bar-container">
        ...
        <ul class="admin-navbar-nav admin-navbar-nav-right">
            <li class="admin-bar-dropdown">
                <a href="{{ route('user.profile.view', ['id' => Sentinel::getUser()->getKey()]) }}" class="dropdown-toggle">
                    {{ Auth::user()->getFullName() }}
                </a>
                <ul class="admin-bar-dropdown-menu">
                    <li><a href="{{ route('user.profile.view', Sentinel::getUser()->getKey()) }}"><i class="icon-user"></i> {{ trans('layouts.profile') }}</a></li>
                    <li><a href="{{ route('access.logout') }}">{{ trans('layouts.logout') }}</a></li>
                </ul>
            </li>
        </ul>
    </div>
