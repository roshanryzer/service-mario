<div class="wrapper-filter">
        ...
        <div class="filter-item form-filter">
            <div class="ui-select-wrapper">
                <select name="filter_columns[]" class="ui-select filter-column-key">
                    <option value="">{{ trans('general.select_field') }}</option>
            ...
            <div class="ui-select-wrapper">
                <select name="filter_operators[]" class="ui-select filter-operator filter-column-operator">
                    <option value="=">{{ trans('general.is_equal_to') }}</option>
                    <option value=">">{{ trans('general.greater_than') }}</option>
                    <option value="<">{{ trans('general.less_than') }}</option>
                </select>
                <svg class="svg-next-icon svg-next-icon-size-16">
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
                </svg>
            ...
            <span class="filter-column-value-wrap">
                <input class="form-control filter-column-value" type="text" placeholder="{{ trans('general.value') }}"
                       name="filter_values[]">
            </span>
            <span class="btn-remove-filter-item" title="{{ trans('general.delete') }}">
    ...
    {{ Form::open(['method' => 'GET', 'class' => 'filter-form']) }}
                ...
                <div class="filter-item form-filter @if ($loop->first) filter-item-default @endif">
                    ...
                    <div class="ui-select-wrapper">
                        <select name="filter_operators[]" class="ui-select filter-column-operator">
                            <option value="="
                                    @if ($filter_item['operator'] == '=') selected @endif>{{ trans('general.is_equal_to') }}</option>
                            <option value=">"
                                    @if ($filter_item['operator'] == '>') selected @endif>{{ trans('general.greater_than') }}</option>
                            <option value="<"
                                    @if ($filter_item['operator'] == '<') selected @endif>{{ trans('general.less_than') }}</option>
                        </select>
                        <svg class="svg-next-icon svg-next-icon-size-16">
                            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
                        </svg>
                    ...
                    @if ($loop->first)
                        <span class="btn-reset-filter-item" title="{{ trans('general.reset') }}">
        ...
        <div style="margin-top: 10px;">
            <a href="javascript:;" class="btn btn-secondary add-more-filter">{{ trans('general.add_additional_filter') }}</a>
            <a href="{{ URL::current() }}" class="btn btn-info @if (!request()->has('filter_table_id')) hidden @endif">{{ trans('general.reset') }}</a>
            <button type="submit" class="btn btn-primary btn-apply">{{ trans('general.apply') }}</button>
        </div>
