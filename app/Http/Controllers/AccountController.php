...
class AccountController extends Controller
{
    ...
    public function dashboard()
    {
        try{
            ...
            return view('account.dashboard',compact('providers','agent','scheduled_rides','service','rides','user_cancelled','provider_cancelled','cancel_rides','revenue'));
        }
        catch(Exception $e){
    ...
    public function statement_agent(){
        ...
        try{
            ...
            foreach($Agents as $index => $Agent){
                ...
                $Rides = UserRequestPayment::where('agent_id',$Agent->id)->get()->pluck('id');

                $Agents[$index]->rides_count = $Rides->count();

                $Agents[$index]->payment = UserRequestPayment::where('agent_id', $Agent->id)
                                ->select(\DB::raw(
                                   'SUM(agent) as overall' 
                                ))->get();                                  
            }

            return view('account.providers.agent-statement', compact('Agents','pagination'))->with('page','Agents Statement');
